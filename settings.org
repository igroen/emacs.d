...
** Tweaks
   #+BEGIN_SRC emacs-lisp
     ...
     (defun settings.org ()
       ...
       (interactive)
       (find-file-other-window (expand-file-name "settings.org" user-emacs-directory)))
     (global-set-key (kbd "C-c s") 'settings.org)
   #+END_SRC
** Backups
   #+BEGIN_SRC emacs-lisp
...
*** org-reveal
   ...
   So clone org-reveal in lisp directory first:
